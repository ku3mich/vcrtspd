...
public class RtspServer : IDisposable
{
    ...
    const int video_payload_type = 96; // = user defined payload, required for H264
    byte[] raw_sps = null;
    byte[] raw_pps = null;
    ...
    public RtspServer(int portNumber, string username, string password, ILoggerFactory loggerFactory)
    {
        ...
        logger = loggerFactory.CreateLogger<RtspServer>();
    ...
    private void AcceptConnection()
    {
        try
        {
            while (!_Stopping.WaitOne(0))
            {
                ...
                logger.LogDebug("Connection from " + oneClient.Client.RemoteEndPoint.ToString());
        ...
        {
            logger.LogWarning("Got an error listening, I have to handle the stopping which also throw an error\n{0}", ex);
        }
        catch (Exception ex)
        {
    ...
    private void RTSP_Message_Received(object sender, RtspChunkEventArgs e)
    {
        ...
        logger.LogDebug("RTSP message received " + message);
        ...
        if (message is Rtsp.Messages.RtspRequestDescribe)
        {
            ...
            logger.LogDebug("Request for " + requested_url);
        ...
        if (message is Rtsp.Messages.RtspRequestSetup)
        {
                ...
                && transport.IsMulticast == false)
            {
                ...
                if (udp_supported)
                {
                    ...
                    {
                        // RTCP data received
                        logger.LogDebug("RTCP data received " + local_sender.ToString() + " " + local_e.ToString());
        ...
        lock (rtspConnectionList)
        {
            ...
            { // Convert to Array to allow us to delete from rtsp_list
                ...
                if (sending_rtp_via_tcp == false && ((now - connection.time_since_last_rtsp_keepalive).TotalSeconds > timeout_in_seconds))
                {
                    ...
                    logger.LogDebug("Removing session " + connection.session_id + " due to TIMEOUT");
    ...
    public void FeedInRawNAL(uint timestamp_ms, List<byte[]> nal_array)
    {
        ...
        // logger.LogDebug(current_rtsp_count + " RTSP clients connected. " + current_rtsp_play_count + " RTSP clients in PLAY mode");
        ...
        lock (rtspConnectionList)
        {
                                                                                ...
                                                                                // eg when there is Write Error
            {
                ...
                logger.LogDebug("Sending video session " + connection.session_id + " " + connection_type + " Timestamp(ms)=" + timestamp_ms + ". RTP timestamp=" + rtp_timestamp + ". Sequence=" + connection.video.sequence_number);
                ...
                if (connection.video.must_send_rtcp_packet)
                {
                    ...
                    if (connection.video.transport_reply.LowerTransport == Rtsp.Messages.RtspTransport.LowerTransportType.TCP)
                    {
                        ...
                        {
                            logger.LogDebug("Error writing RTCP Sender Report to listener " + connection.listener.RemoteAdress);
                        }
                    }
                    ...
                    if (connection.video.transport_reply.LowerTransport == Rtsp.Messages.RtspTransport.LowerTransportType.UDP && connection.video.transport_reply.IsMulticast == false)
                    {
                        ...
                        {
                            logger.LogDebug("UDP Write Exception " + e.ToString());
                            logger.LogDebug("Error writing RTCP to listener " + connection.listener.RemoteAdress);
                        }
                    }
                ...
                foreach (byte[] rtp_packet in rtp_packets)
                {
                    ...
                    if (connection.video.transport_reply.LowerTransport == Rtsp.Messages.RtspTransport.LowerTransportType.TCP)
                    {
                        ...
                        {
                            logger.LogDebug("Error writing to listener " + connection.listener.RemoteAdress);
                            write_error = true;
                ...
                if (write_error)
                {
                    logger.LogDebug("Removing session " + connection.session_id + " due to write error");
                    connection.play = false; // stop sending data
                    if (connection.video.udp_pair != null)
                    {
                        connection.video.udp_pair.Stop();
    ...
    public void FeedInAudioPacket(uint timestamp_ms, byte[] audio_packet)
    {
        ...
        lock (rtspConnectionList)
        {
                                                                                ...
                                                                                // eg when there is Write Error
            {
                ...
                logger.LogDebug(
                    "Sending audio session " + connection.session_id + " " + connection_type +
                    " Timestamp(ms)=" + timestamp_ms +
                    ". RTP timestamp=" + rtp_timestamp +
                    ". Sequence=" + connection.audio.sequence_number);
                ...
                connection.audio.rtp_packet_count += (uint)rtp_packets.Count;
